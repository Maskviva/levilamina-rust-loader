...
想让某个世界不刷怪而其他世界照常，gamerule 做不到——它没有维度维度。要按维度隔离用 [Dimensions](/api/world/dimensions) 的 `set_dimension_rule`。
:::
...
| `s.scan_region(dim, a, b)` | 扫描一块区域，见 [World](/api/world/world) |

单个方块的完整操作（方块状态、方块实体、tag）用 [`Block`](/api/world/block)。
...
- 执行和注册命令 → [Command](/api/actor/command)
- `subscribe_event` / `list_events` → [Event](/api/infra/event)
- `schedule` / `schedule_after` / `cancel_task` / `pending_tasks` → [Scheduler](/api/rt/scheduler)
- `spawn_sim_player` / `sim_player` / `is_simulated` / `list_sim_players` → [SimPlayer](/api/actor/sim)
