...
`local_player()` 返回的是普通的 [`Player`](/api/actor/player) 句柄，整套玩家 API 都能用，包括 `.get_actor()`。
...
- [`Block`](/api/world/block) / [`Actor`](/api/actor/entity) / [`Player`](/api/actor/player) / [`ItemStack`](/api/world/item) / [`Container`](/api/world/container)
- [`NbtValue`](/api/world/nbt) / [`KvDb`](/api/infra/data) / [`system`](/api/core/system) / [`Logger`](/api/rt/log)
- [`bus`](/api/core/bus) / [`service`](/api/core/service)
- [`packets()`](/api/infra/packet)（ABI 槽位排在客户端专属块之前，两边都提供）
