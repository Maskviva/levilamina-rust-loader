...
`money::on_before` / `on_after` 返回的 guard 也是同样的用法，但语义特殊，见 [Money](/api/money)。

## 枚举速查

| 枚举 | 取值 | 在哪 |
| --- | --- | --- |
| `EventPriority` | Highest=0 High=1 Normal=2 Low=3 Lowest=4 | [Event](/api/event) |
| `LogLevel` | Fatal=0 Error=1 Warn=2 Info=3 Debug=4 Trace=5 | [Log](/api/log) |
| `GameMode` | Survival=0 Creative=1 Adventure=2 Spectator=6 | [Player](/api/player) |
| `MessageType` | Raw=0 … TextObjectAnnouncement=11 | [Player](/api/player) |
| `TitleKind` | Clear=0 Reset=1 Title=2 Subtitle=3 Actionbar=4 Times=5 | [Player](/api/player) |
| `Ability` | Build=0 … VerticalFlySpeed=19 | [Player](/api/player) |
| `CommandPermission` | Any=0 GameDirectors=1 Admin=2 Host=3 Owner=4 | [Command](/api/command) |
| `ParamType` | Int Bool Float String … FilePath（21 种） | [Command](/api/command) |
| `SoftEnumOp` | Set=0 Add=1 Remove=2 | [Command](/api/command) |
| `Weather` | Clear=0 Rain=1 Thunder=2 | [Server](/api/server) |
| `GamingStatus` | Default Starting Running Stopping | [Server](/api/server) |
| `DisplaySlot` | Sidebar List BelowName | [Scoreboard](/api/scoreboard) |
| `FormValue` | Int Float Text Choice | [Gui](/api/gui) |
| `FormResponse` | Cancelled Button Custom Modal | [Gui](/api/gui) |
| `NbtValue` | Byte … LongArray（12 种） | [Nbt](/api/nbt) |
| `NbtBinaryFormat` | Disk=0 Network=1 | [Nbt](/api/nbt) |
| `Direction` | Inbound Outbound Both | [Packet](/api/packet) |
| `Verdict` | Forward Drop | [Packet](/api/packet) |
| `ConnectionState` | Opened Closed | [Packet](/api/packet) |
| `CallError` | NotFound Provider Refused Unknown | [Service](/api/service) |
| `GeneratorType` | Overworld=1 Flat=2 Nether=3 TheEnd=4 Void=5 | [Dimensions](/api/dimensions) |
| `DimensionRule` | SpawnMonster=0 … EntityCrossPlot=12 | [Dimensions](/api/dimensions) |
| `KeyAction` | Up=0 Down=1 | [Client](/api/client) |
