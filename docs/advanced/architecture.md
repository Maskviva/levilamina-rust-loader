...
本页说明整个项目怎么分层、模组如何被加载，以及"为什么长这样"。API 具体有什么见 [API 参考](/api/core/overview)；契约细节与演进规则见 [ABI 契约与演进](/advanced/abi)。
...
`LeviRsAbi.h` 是加载器与模组之间那份契约**唯一的真相来源**——`levilamina-sys` 逐字段镜像它，谁都不能单独改。核心是 `LeviRsApi`：一张扁平的函数指针表，在模组加载时一次性交给 Rust 侧，指针在模组整个生命周期内有效。版本用 `abi_version`（大版本，**按区间判断**：加载器接受不比自己新、不低于下限的模组）+ `struct_size`（前向兼容的精确检查）两个数字把关，演进规则只有一条：**新字段永远追加在表尾，从不重排、从不删除**。
