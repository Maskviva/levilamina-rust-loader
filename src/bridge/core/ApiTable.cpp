...
namespace levi_rs
{
    namespace
    {
        ...
        const LeviRsApi gApi{
            ...
            /* ── Rust 高速公路（Lane.cpp）── */
            /* lane_publish               */ api_lane_publish,
            /* lane_unpublish             */ api_lane_unpublish,
            /* lane_acquire               */ api_lane_acquire,
            /* lane_release               */ api_lane_release,
            /* lane_list                  */ api_lane_list,

            /* ── 追加：既有维度枚举（MoreDimensionsBridge.cpp）── */
            /* md_list_dimensions         */ api_md_list_dimensions,
            /* level_delete_chunk_keys    */ api_level_delete_chunk_keys,
            /* level_chunks_loaded        */ api_level_chunks_loaded,
            /* player_conn_id             */ api_player_conn_id,
    ...
    namespace detail
    {
        void onRustModGone(RustMod* mod)
        {
            ...
            // 车道紧跟 services，理由再进一层：service 悬垂的是一个回调指针，
            // 车道悬垂的是**整张函数表**。这一步必须在 lib.free() 之前跑完
            // ——它要替未归还的租约调用提供方的 release，而那段代码就在即将被
            // unmap 的那个 dylib 里。
            bridge::laneOnRustModGone(mod);
