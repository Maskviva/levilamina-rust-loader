...
namespace levi_rs::bridge
{
    ...
    )
    {
        ...
        case LEVI_RS_AACT_EXECUTE_EVENT:
            // Fires a behaviour-pack entity event by name — the same thing
            // `/event entity <target> <event>` does. This is how a mod changes
            // anything that only exists as a component group (scale, hitbox,
            // AI swaps): there is no setter for those, and there is no synched
            // data id for scale in this game version either.
            if (sarg.empty()) return false;
            return actor->executeEvent(std::string{sarg}, VariantParameterList{});
        case LEVI_RS_AACT_SET_ROTATION:
            // Vec2 is (x = pitch, y = yaw) — same order the ROT_* props read.
            // Wrapped, not Directly: setRotationDirectly skips the -180..180
            // normalisation and a yaw of 400 then renders as a spun-around head.
