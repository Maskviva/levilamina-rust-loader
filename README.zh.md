...
- **一等公民的 mod。** Rust mod 住在 `mods/<name>/`，带一份普通的
...
2. 把 `levilamina-rust-loader` 的发布包放进 `mods/levilamina-rust-loader/`
   （或者自己编译，见下文）。
3. 把 Rust mod 装进 `mods/<mod名>/`——跟其他任何 LeviLamina mod 一样，都是一个
